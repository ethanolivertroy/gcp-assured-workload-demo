...
**Key Finding**: Assured Workloads prevents only **4 out of 12** attempted violations (33%). **8 violations** (67%) are still possible, proving that service-level configuration is critical for compliance.

### What Assured Workloads Actually Prevents

✅ **Blocks 4 violations** (via organization policies):
1. Storage buckets without UBLA (`constraints/storage.uniformBucketLevelAccess`)
2. Public bucket access via IAM (`allUsers` / `allAuthenticatedUsers` bindings blocked)
3. Service account key creation (`constraints/iam.disableServiceAccountKeyCreation`)
4. Overprivileged IAM roles like Editor (IAM policy restrictions)

❌ **Allows 8 violations** (service-level misconfigurations):
